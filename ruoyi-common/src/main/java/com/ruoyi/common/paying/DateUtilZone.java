package com.pig4cloud.pigx.thirdpartygames.util;
...
// e.printStackTrace();
    ...
    public static Date[] getDatetimeMonthLimit(String monthStr) {
        ...
        Date startTime = DateUtilZone.format(start);
    ...
    public static Date[] getDatetimeDayLimit(String day) {
        ...
        Date start = DateUtilZone.parseDate(startStr);
    ...
    public static Date[] getDatetimePreDayLimit(String day) {
        String startStr = day + " 00:00:00";
        Date start = new Date(DateUtilZone.parseDate(startStr).getTime() - 24 * 60 * 60 * 1000l);
        return new Date[]{start, new Date(start.getTime() + 24 * 60 * 60 * 1000l - 1l)};
    }
    ...
    public static int getIntervalOfDays(Date fDate, Date oDate) {
        ...
        fDate = DateUtilZone.clearDate(fDate, 4).getTime();
        oDate = DateUtilZone.clearDate(oDate, 4).getTime();
    ...
    /***
     * 把 时间戳转换为  yyyy-MM-dd HH:mm:ss 时间格式
     * @param time
     * @return
     */
     ...
     * @return
     */
    public static Long Unx(String time) {
    ...
    public static Long UnxUTC(String time) {
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSS'-04:00'");
        Date date = null;
        long timestamp = 0;
        try {
            date = format.parse(time);
    ...
    /**
     * 将 dateTime 转换为  yyyy-MM-dd HH:mm:ss
     *
     * @param
     * @return
     */
    public static String dateStringFormat(DateTime dateTime) {
        String stringFormat = "";
        try {
            stringFormat = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(dateTime);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return stringFormat;
    ...
    public static Date timestampToDate(Integer time) {
        String stringTime = timestampToString(time);
        //将 string转换为date 类型
        Date format = format(stringTime);
        return format;
    }

    public static String timestampToStringLebo(Long time) {
        //int转long时，先进行转型再进行计算，否则会是计算结束后在转型
        long temp = time * 1000;
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");//要转换的时间格式
        Date date;
    ...
    public static String timestampToStringOb(Long time) {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");//要转换的时间格式
        Date date;
        try {
    ...
    /**
     * 把 String类型 转换为 DateTime 类型
     *
     * @param time 传入String 类型的时间
     * @return
     */
    public static DateTime stringUpDateTime(String time) {
        DateTime fromTime;
        try {
            //转换为时间戳
            long timeZx = DateUtilZone.format(time).getTime();
            fromTime = new DateTime(timeZx);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static int StringFormatInt(String time) {
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        int timestamp = 0;
        try {
            timestamp = (int) ((Timestamp.valueOf(time).getTime()) / 1000);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return timestamp;
     ...
     * 将时间转换为时间戳
     */
    public static Long timeZone(DateTime dateTime) {
        String s = dateStringFormat(dateTime);
        Long unx = Unx(s);
        return unx;
    }


    public static boolean getIsEnded(String[] createTime) {
        SimpleDateFormat format = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
        if (CollectionUtils.isEmpty(Arrays.asList(createTime)) || createTime.length != 2) {
            return false;
        }
        Long currentTime = System.currentTimeMillis();
        Long endD = null;
        try {
            format.parse(createTime[0]);
            format.parse(createTime[1]);

            endD = format.parse(createTime[1]).getTime();
    ...
    public static boolean getIsCurrentWeek(String[] createTime) {
        if (CollectionUtils.isEmpty(Arrays.asList(createTime)) || createTime.length != 2) {
            return false;
        }
        Calendar calendar = Calendar.getInstance();
        ...
        String mondayStr = new SimpleDateFormat("yyyy-MM-dd").format(monday);

        calendar.add(Calendar.DAY_OF_MONTH, 6);

        Date sunday = calendar.getTime();
        String sundayStr = new SimpleDateFormat("yyyy-MM-dd").format(sunday);


        return createTime[0].contains(mondayStr) && createTime[1].contains(sundayStr);
    }

    public static boolean getIsLastWeek(String[] createTime) {
        if (CollectionUtils.isEmpty(Arrays.asList(createTime)) || createTime.length != 2) {
            return false;
        }
        Calendar calendar = Calendar.getInstance();
     ...
     * @return
     */
    public static Integer[] getMonths(String[] createTime) {
        Map<Integer, Integer> monthsInMap = Arrays.asList(createTime).stream().map(item -> {
            return Integer.parseInt(item.substring(5, 7));
        }).collect(Collectors.toMap(Integer::intValue, v -> v, (v1, v2) -> v1));

        if (CollectionUtils.isEmpty(monthsInMap)) {
            return null;
        }

        List<Integer> monthsInList = monthsInMap.entrySet().stream().map(entry -> {
            return entry.getKey();
        }).collect(Collectors.toList());

        return monthsInList.toArray(new Integer[0]);
    }
    ...
    public static Date getLocalTime(Date date) {
        ...
        DateTime startTime = DateUtil.offsetMinute(dateTime, -60);
        System.out.println(startTime);
        String startTimeStr = DateUtil.truncate(startTime, DateField.MINUTE).toString("YYYY-MM-dd HH:mm:ss");
        Date format = DateUtilZone.format(startTimeStr);
     ...
     * 获取GMT+8的时间-n分钟的时间戳
     */
    public static Long getLocalTimeSystemCurrentTimeMillis(int n) {
        //获取当前时间-n分钟的时间
     ...
     * 把时间戳转换为Date时间类型
     */
    public static Date getLocalDateTime(Long time) {
        String result = timeStamp2Date(String.valueOf(time));
        //转换为Date类型
        Date date = format(result);
        return date;
    }
     ...
     * 格式转换
     */
    public static Date getJlLocalTime(String time) {
        if (time.contains("T")) {
            time = time.replace("T", " ");
            if (time.contains("-04:00")) {
                time = time.replace("-04:00", "");
            }
        }
        System.out.println(time);
        Date format = DateUtilZone.format(time);
        System.out.println(format);
        DateTime dateTime = new DateTime(format);
        DateTime startTime = DateUtil.offsetHour(dateTime, 11);
        System.out.println(startTime);
        String startTimeStr = DateUtil.truncate(startTime, DateField.MINUTE).toString("YYYY-MM-dd HH:mm:ss");
        Date date = DateUtilZone.format(startTimeStr);
        System.out.println(date);
        return date;
    }

    /**
     * 把时间转换为 yyyymmdd’T’24hhmmss 格式
     */
    public static String getNextspinTime(String time) {
    ...
    public static Date getDateNextspin(String time) {
        if (time.contains("T")) {
            time = time.replace("T", StrUtil.EMPTY);
            String convertion = dateConvertion(time);
            Date format = format(convertion);
            System.out.println(format);
            return format;
        }
        return new Date();
    ...
    public static String getDateNextspinString(DateTime start) {
        //先转换成 yyyymmdd24hhmmss
        //1.转换成 new DATE
        Date localDateTime = getLocalDateTime(start.getTime());
        //2.转换成String
        String format = format(localDateTime);
        //3.转换成yyyymmdd24hhmmss
        String timeFront = getTimeFront(format);
        return timeFront;
    }
